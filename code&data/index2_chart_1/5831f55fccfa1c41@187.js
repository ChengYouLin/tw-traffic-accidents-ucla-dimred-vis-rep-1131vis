function _1(md){return(
  ...
  function _chart(d3, data) {
    ...
    // Create the color scale with gray and white tones.
    const color = d3.scaleLinear()
        .domain([0, 5])
        .range(["#f7f7f7", "#4d4d4d"]) // Gray-white color scheme
    ...
    const node = svg.append("g")
      ...
      .join("circle")
        ...
        .on("mouseover", function(event, d) {
          ...
          tooltip.html(d.data.name || "Unknown")
    ...
    const label = svg.append("g")
        ...
        .style("font-weight", "bold") // Text is bold
        .style("fill", "#333") // Text color for better contrast
        .attr("pointer-events", "none")
        .attr("text-anchor", "middle")
      .selectAll("text")
      .data(root.descendants())
      .join("text")
        //.style("fill-opacity", d => (d.parent === root && d.r > 30) ? 1 : 0) // Hide text if the node is too small
        .style("display", d => (d.parent === root && d.r > 30) ? "inline" : "none") // Hide when zoomed out or too small
        .attr("dy", "0.35em")
        .text(d => d.data.name);
