...
function _chart(d3, width, height, x, root, up, xAxis, yAxis, down) {
    ...
    .attr("width", 800)  // 設定固定寬度
    .attr("height", 600)
      //.style("background-color", "#f0f0f0"); // 設定背景為淺灰色
  ...
  function _bar(margin, barStep, barPadding, x) {
    return function bar(svg, down, d, selector) {
      const g = svg.insert("g", selector)
        .attr("class", "enter")
        .attr("transform", `translate(0,${margin.top + barStep * barPadding})`)
        .attr("text-anchor", "end")
        .style("font", "bold 12px sans-serif"); // 調整字型為粗體且加大兩個字號
...
d3.scaleLinear().range([margin.left, 800 - margin.right])
)}
...
g => g
    ...
    .call(d3.axisTop(x).ticks(800 / 80, "s"))
...
d3.scaleOrdinal([true, false], ["#5151A2", "#aaa"])
)}
